...
func (h *SyncHandler) notifySharedUsers(apnsClient *apns.Client, userID, deviceID string, carIDs []string) {
	...
	for _, dt := range tokens {
		...
		if remove {
			h.store.LogNotification(ctx, dt.UserID, dt.DeviceID, tokenPrefix, "background", false, "token_invalid")
			if err := h.store.DeleteDeviceTokenByToken(ctx, dt.Token); err != nil {
				log.Printf("sync: cleanup invalid token: %v", err)
			}
		} else {
			h.store.LogNotification(ctx, dt.UserID, dt.DeviceID, tokenPrefix, "background", true, "")
		}
	}
}
