...
func (s *Store) CreateMagicTokenWithPoll(ctx context.Context, userID, token, pollToken string, expiresAt time.Time) error {
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO magic_tokens (user_id, token, poll_token, expires_at) VALUES ($1, $2, $3, $4)`,
		userID, token, pollToken, expiresAt)
	return err
}
...
func (s *Store) GetMagicToken(ctx context.Context, token string) (*model.MagicToken, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT id, user_id, token, poll_token, expires_at, used, created_at FROM magic_tokens WHERE token = $1`, token).
		Scan(&t.ID, &t.UserID, &t.Token, &t.PollToken, &t.ExpiresAt, &t.Used, &t.CreatedAt)
...
func (s *Store) GetMagicTokenByPollToken(ctx context.Context, pollToken string) (*model.MagicToken, error) {
	t := &model.MagicToken{}
	err := s.db.QueryRowContext(ctx,
		`SELECT id, user_id, token, poll_token, expires_at, used, created_at FROM magic_tokens WHERE poll_token = $1`, pollToken).
		Scan(&t.ID, &t.UserID, &t.Token, &t.PollToken, &t.ExpiresAt, &t.Used, &t.CreatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
		return nil, fmt.Errorf("get magic token by poll token: %w", err)
	}
	return t, nil
}
